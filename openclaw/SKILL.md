...
| `upload_reference_image` | Compress and validate a user-specified image (max 2MB, 2048px) for use as a style reference. The tool checks file format, resizes if needed, and returns a temporary URL (expires in 24 hours). ComfyUI users can skip this — pass local file paths directly to `generate_image`. |
...
Use an existing image to guide visual style. The `upload_reference_image` tool handles compression and format validation automatically.
...
**Important**: If `upload_reference_image` fails or is unavailable, ask the user to provide an image URL directly (e.g., from [meigen.ai](https://www.meigen.ai) or any publicly accessible link).
