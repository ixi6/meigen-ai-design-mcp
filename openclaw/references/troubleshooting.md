...
-> Use the `upload_reference_image` tool — it handles compression, size validation, and automatic expiry as a single validated pipeline. Do not reimplement these checks manually. If the tool is unavailable, ask the user to provide an image URL directly instead.
